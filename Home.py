...
with st.sidebar:
    ...
    if "dataset" in st.session_state and st.session_state["dataset"] is not None:
        ...
        st.markdown(f'<span class="badge badge-loaded">✔ Dataset Loaded</span> &nbsp; {n_rows:,} × {n_cols}', unsafe_allow_html=True)
    else:
...
with col_upload:
    ...
    if uploaded_file is not None:
        try:
            ...
            log_to_report("Dataset", f"<p>Uploaded custom dataset: <strong>{uploaded_file.name}</strong> ({df.shape[0]:,} rows × {df.shape[1]} cols)</p>")
    ...
    if st.button("🚀 Load Default Dataset", use_container_width=True):
        with st.spinner("Loading & preprocessing..."):
            ...
            if df is not None:
                ...
                rfm = derive_rfm(cleaned)
                st.session_state["rfm_data"] = rfm
                st.success(f"✅ Loaded **Online Retail** — {df.shape[0]:,} rows × {df.shape[1]} columns")
                log_to_report("Dataset", f"<p>Loaded default Online Retail dataset ({df.shape[0]:,} rows × {df.shape[1]} cols)</p>")
            else:
